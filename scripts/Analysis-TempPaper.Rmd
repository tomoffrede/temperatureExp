...
tidy(m <- nnet::multinom(effect ~ becomeFriendsC, data=d |> filter(ROI=="Nose"))) |> 
  as_tibble() |> 
  mutate(estimateP = plogis(estimate)) #  transforms log odds (the output of the model) into probabilities
...
temp <- d |> filter(ROI=="Nose")
m <- nnet::multinom(effect ~ extraversionC, data=temp)
lmtest::lrtest(m)
```
...
temp <- d |> filter(ROI=="Eyes")
m <- nnet::multinom(effect ~ agreeablenessC, data=temp, model=TRUE)
lmtest::lrtest(m)
```
...
temp <- d |> filter(ROI=="Forehead")
m <- nnet::multinom(effect ~ likeabilityC, data=temp, model=TRUE)
lmtest::lrtest(m)
```
