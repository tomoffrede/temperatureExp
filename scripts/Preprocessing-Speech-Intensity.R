# same script as "Preprocessing-Speech.R", but also saving intensity data
...
names(f0) <- c("file", "speaker", "turn", "IPU", "f0mean", "f0med", "f0sd", "f0max", "intensity", "turnOnset", "turnOffset", "ipuOnset", "ipuOffset", "task", "firstSp")
f0 <- f0 |> 
  mutate_at(c("turn", "IPU", "f0mean", "f0med", "f0sd", "f0max", "intensity", "turnOnset", "turnOffset", "ipuOnset", "ipuOffset"), as.numeric)
...
for(i in 1:nrow(files)){
  ...
  txt <- read.table(paste0(folderAll, files$TXT[i]), header = TRUE, na.strings = "--undefined--") |> 
    mutate(meanZ = (f0mean - mean(f0mean, na.rm=TRUE)) / sd(f0mean, na.rm=TRUE),
           ...
           intZ = (intensityMean - mean(intensityMean, na.rm=TRUE)) / sd(intensityMean, na.rm=TRUE)) |> 
    mutate(f0mean = ifelse(abs(meanZ) > 2.5, NA, f0mean),
           f0med = ifelse(abs(medZ) > 2.5, NA, f0med),
           f0sd = ifelse(abs(sdZ) > 2.5, NA, f0sd),
           f0max = ifelse(abs(maxZ) > 2.5, NA, f0max),
           intensityMean = ifelse(abs(intZ) > 2.5, NA, intensityMean))
      ...
      for(s in 1:tg.getNumberOfIntervals(tg, silenceTier)){
        if(tg.getLabel(tg, silenceTier, s)=="sounding"){
          ...
          if((startIPU >= turnOnset & startIPU < turnOffset) | (endIPU > turnOnset & endIPU <= turnOffset)){
            ipuCount <- ipuCount + 1
            f <- data.frame(matrix(nrow=0, ncol=5))
            names(f) <- c("f0mean", "f0med", "f0sd", "f0max", "intensity")
            for(l in 1:nrow(txt)){
...
dat <- merge(f0, m |> select(speaker, gender), by="speaker") |> 
  mutate_at(c("f0mean", "f0med", "f0sd", "f0max", "intensity", "turn", "IPU", "turnOnset", "turnOffset", "ipuOnset", "ipuOffset"), as.numeric) |>
  ...
  mutate(f0mean = ifelse(is.nan(f0mean), NA, f0mean),
         ...
         intensity = ifelse(is.nan(intensity), NA, intensity),
         ...
         intz = (intensity - mean(intensity, na.rm=TRUE)) / sd(intensity, na.rm=TRUE)) |> 
  ungroup() |> 
  group_by(gender) |> 
  mutate(f0meanzGender = (f0mean - mean(f0mean, na.rm=TRUE)) / sd(f0mean, na.rm=TRUE),
         ...
         intzGender = (intensity - mean(intensity, na.rm=TRUE)) / sd(intensity, na.rm=TRUE)) |> 
  ungroup() |> 
  ...
  mutate(turnDur = turnOffset - turnOnset,
         ...
         turnInt = ifelse(is.nan(turnInt), NA, turnInt),
         firstSp = NA,
         prevf0mean = NA,
         prevf0med = NA,
         prevf0sd = NA,
...
save(dat, file=paste0(folderData, "speechData-intensity.RData"))
save(ipus, file=paste0(folderData, "speechData-allIPUs-intensity.RData"))
