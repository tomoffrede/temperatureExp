...
summary(m1 <- lmer(f0sdz ~ turnNormalTask : condition + (1+turnNormalTask|speaker), ipus |> filter(task=="Lists")))
anova(m, m1)
par(mfrow=c(2, 2))
hist(resid(m))
...
Below are the effects of temperature change per ROI on f0 SD change. During *Lists*, changes in eye and forehead temperature predicted smaller f0 SD changes.
```{r echo=FALSE}
print(tos |> filter(temperatureAffectsF0=="affects", section=="Lists") |> arrange(ROI))
```
