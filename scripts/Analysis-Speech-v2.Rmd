...
* First section of the experiment (Lists)
    * f0 median didn't change significantly
    * f0 max decreased only in the *impersonal* condition
    * f0 SD decreased in both conditions
* Second section of the experiment (Diapix)
    * f0 median increased only in the *close* condition (but model without `condition` preferred)
    * f0 max didn't change significantly
    * f0 SD
* Across entire experiment
    * f0 median increased only in the *close* condition
    * f0 max decreased in both conditions
    * f0 SD increased only in the *close* condition
...
for(s in unique(ipus$speaker)){
  # f0 median
  c <- tidy(lm(f0med ~ turnOverall, ipus |> filter(speaker==s)))
  ind[nrow(ind)+1,] <- c(s, unique(ipus$condition[ipus$speaker==s]), "f0median", "entireExp",
                         c$statistic[2], c$p.value[2])
  c <- tidy(lm(f0med ~ turnOverall, ipus |> filter(speaker==s, task=="Lists")))
  ind[nrow(ind)+1,] <- c(s, unique(ipus$condition[ipus$speaker==s]), "f0median", "Lists",
                         c$statistic[2], c$p.value[2])
  if(!all(is.na(ipus$f0med[ipus$speaker==s & ipus$task=="Diapix"]))){ # need to do this because there are missing files for one dyad for this section
    c <- tidy(lm(f0med ~ turnOverall, ipus |> filter(speaker==s, task=="Diapix")))
    ind[nrow(ind)+1,] <- c(s, unique(ipus$condition[ipus$speaker==s]), "f0median", "Diapix",
                           c$statistic[2], c$p.value[2])
  }
  
  # f0 max
  c <- tidy(lm(f0max ~ turnOverall, ipus |> filter(speaker==s)))
  ind[nrow(ind)+1,] <- c(s, unique(ipus$condition[ipus$speaker==s]), "f0max", "entireExp",
                         c$statistic[2], c$p.value[2])
  c <- tidy(lm(f0max ~ turnOverall, ipus |> filter(speaker==s, task=="Lists")))
  ind[nrow(ind)+1,] <- c(s, unique(ipus$condition[ipus$speaker==s]), "f0max", "Lists",
                         c$statistic[2], c$p.value[2])
  if(!all(is.na(ipus$f0max[ipus$speaker==s & ipus$task=="Diapix"]))){ # need to do this because there are missing files for one dyad for this section
    c <- tidy(lm(f0max ~ turnOverall, ipus |> filter(speaker==s, task=="Diapix")))
    ind[nrow(ind)+1,] <- c(s, unique(ipus$condition[ipus$speaker==s]), "f0max", "Diapix",
                           c$statistic[2], c$p.value[2])
  }
  
  # f0 SD
  c <- tidy(lm(f0sd ~ turnOverall, ipus |> filter(speaker==s)))
  ind[nrow(ind)+1,] <- c(s, unique(ipus$condition[ipus$speaker==s]), "f0SD", "entireExp",
                         c$statistic[2], c$p.value[2])
  c <- tidy(lm(f0sd ~ turnOverall, ipus |> filter(speaker==s, task=="Lists")))
  ind[nrow(ind)+1,] <- c(s, unique(ipus$condition[ipus$speaker==s]), "f0SD", "Lists",
                         c$statistic[2], c$p.value[2])
  if(!all(is.na(ipus$f0sd[ipus$speaker==s & ipus$task=="Diapix"]))){ # need to do this because there are missing files for one dyad for this section
    c <- tidy(lm(f0sd ~ turnOverall, ipus |> filter(speaker==s, task=="Diapix")))
...
```{r}
tidy(m <- nnet::multinom(change ~ condition, data=dat |> filter(feature=="f0median"))) |> 
  as_tibble() |> 
...
```{r}
tidy(m <- nnet::multinom(change ~ condition, data=dat |> filter(feature=="f0max"))) |> 
  as_tibble() |> 
  mutate(estimate = plogis(estimate)) # plogis transforms log odds (the output of the model) into probabilities
emmeans(m, pairwise~condition, by="change")
```
...
```{r}
tidy(m <- nnet::multinom(change ~ condition, data=dat |> filter(feature=="f0SD"))) |> 
  as_tibble() |> 
  mutate(estimate = plogis(estimate)) # plogis transforms log odds (the output of the model) into probabilities
emmeans(m, pairwise~condition, by="change")
```
...
-   Across entire experiment: f0 median increases only in *close* condition
-   Only during question lists: f0 median doesn't change in any group.
-   Only during Diapix: f0 median tends to go up only in the *close* condition, BUT the model with `condition` was *not* preferred (even though the difference in the graph is huge)
...
```{r}
ggplot(ipus, aes(turnOverall, f0med))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~condition)+
  ggtitle("Entire experiment")
summary(m <- lmer(f0med ~ turnOverall + (1|speaker), ipus))
summary(m1 <- lmer(f0med ~ turnOverall : condition + (1|speaker), ipus))
anova(m, m1)
par(mfrow=c(2, 2))
hist(resid(m1))
...
ggplot(ipus |> filter(task=="Lists"), aes(turnOverall, f0med))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~condition)+
  ggtitle("Question lists (1st part of experiment)")
summary(m <- lmer(f0med ~ turnOverall + (1|speaker), ipus |> filter(task=="Lists")))
summary(m1 <- lmer(f0med ~ turnOverall : condition + (1|speaker), ipus |> filter(task=="Lists")))
anova(m, m1)
par(mfrow=c(2, 2))
hist(resid(m))
...
ggplot(ipus |> filter(task=="Diapix"), aes(turnOverall, f0med))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~condition)+
  ggtitle("Diapix (2nd part of experiment)")
summary(m <- lmer(f0med ~ turnOverall + (1|speaker), ipus |> filter(task=="Diapix")))
summary(m1 <- lmer(f0med ~ turnOverall : condition + (1|speaker), ipus |> filter(task=="Diapix")))
anova(m, m1)
par(mfrow=c(2, 2))
hist(resid(m))
qqnorm(resid(m));qqline(resid(m))
...
```{r}
summary(m <- lmer(f0med ~ turnOverall + (1|speaker), ipus))
summary(m1 <- lmer(f0med ~ turnOverall : closeness + (1|speaker), ipus))
summary(m2 <- lmer(f0med ~ turnOverall : similarity + (1|speaker), ipus))
summary(m3 <- lmer(f0med ~ turnOverall : likeability + (1|speaker), ipus))
summary(m4 <- lmer(f0med ~ turnOverall : becomeFriends + (1|speaker), ipus))
...
-   Only during question lists: f0 max decreases only in the *impersonal* condition
...
```{r}
ggplot(ipus, aes(turnOverall, f0max))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~condition)+
  ggtitle("Entire experiment")
summary(m <- lmer(f0max ~ turnOverall + (1|speaker), ipus))
summary(m1 <- lmer(f0max ~ turnOverall : condition + (1|speaker), ipus))
anova(m, m1)
par(mfrow=c(2, 2))
hist(resid(m))
qqnorm(resid(m));qqline(resid(m))
...
ggplot(ipus |> filter(task=="Lists"), aes(turnOverall, f0max))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~condition)+
  ggtitle("Question lists (1st part of experiment)")
summary(m <- lmer(f0max ~ turnOverall + (1|speaker), ipus |> filter(task=="Lists")))
summary(m1 <- lmer(f0max ~ turnOverall : condition + (1|speaker), ipus |> filter(task=="Lists")))
anova(m, m1)
par(mfrow=c(2, 2))
hist(resid(m1))
qqnorm(resid(m1));qqline(resid(m1))
...
ggplot(ipus |> filter(task=="Diapix"), aes(turnOverall, f0max))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~condition)+
  ggtitle("Diapix (2nd part of experiment)")
summary(m <- lmer(f0max ~ turnOverall + (1|speaker), ipus |> filter(task=="Diapix")))
summary(m1 <- lmer(f0max ~ turnOverall : condition + (1|speaker), ipus |> filter(task=="Diapix")))
anova(m, m1)
par(mfrow=c(2, 2))
hist(resid(m))
qqnorm(resid(m));qqline(resid(m))
...
```{r}
summary(m <- lmer(f0max ~ turnOverall + (1|speaker), ipus))
summary(m1 <- lmer(f0max ~ turnOverall : closeness + (1|speaker), ipus))
summary(m2 <- lmer(f0max ~ turnOverall : similarity + (1|speaker), ipus))
summary(m3 <- lmer(f0max ~ turnOverall : likeability + (1|speaker), ipus))
summary(m4 <- lmer(f0max ~ turnOverall : becomeFriends + (1|speaker), ipus))
...
-   Across entire experiment: f0 SD increases only in the *close* condition
-   Only during question lists: f0 SD goes down in both conditions
-   Only during Diapix: f0 SD doesn't change for either condition
...
```{r}
ggplot(ipus, aes(turnOverall, f0sd))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~condition)+
  ggtitle("Entire experiment")
summary(m <- lmer(f0sd ~ turnOverall + (1|speaker), ipus))
summary(m1 <- lmer(f0sd ~ turnOverall : condition + (1|speaker), ipus))
anova(m, m1)
par(mfrow=c(2, 2))
hist(resid(m1))
qqnorm(resid(m1));qqline(resid(m1))
...
ggplot(ipus |> filter(task=="Lists"), aes(turnOverall, f0sd))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~condition)+
  ggtitle("Question lists (1st part of experiment)")
summary(m <- lmer(f0sd ~ turnOverall + (1|speaker), ipus |> filter(task=="Lists")))
summary(m1 <- lmer(f0sd ~ turnOverall : condition + (1|speaker), ipus |> filter(task=="Lists")))
anova(m, m1)
par(mfrow=c(2, 2))
hist(resid(m))
qqnorm(resid(m));qqline(resid(m))
...
ggplot(ipus |> filter(task=="Diapix"), aes(turnOverall, f0sd))+
  geom_point()+
  geom_smooth(method="lm")+
  facet_wrap(~condition)+
  ggtitle("Diapix (2nd part of experiment)")
summary(m <- lmer(f0sd ~ turnOverall + (1|speaker), ipus |> filter(task=="Diapix")))
summary(m1 <- lmer(f0sd ~ turnOverall : condition + (1|speaker), ipus |> filter(task=="Diapix")))
anova(m, m1)
par(mfrow=c(2, 2))
hist(resid(m))
qqnorm(resid(m));qqline(resid(m))
...
```{r}
summary(m <- lmer(f0sd ~ turnOverall + (1|speaker), ipus))
summary(m1 <- lmer(f0sd ~ turnOverall : closeness + (1|speaker), ipus))
summary(m2 <- lmer(f0sd ~ turnOverall : similarity + (1|speaker), ipus))
summary(m3 <- lmer(f0sd ~ turnOverall : likeability + (1|speaker), ipus))
summary(m4 <- lmer(f0sd ~ turnOverall : becomeFriends + (1|speaker), ipus))
...
```{r}
tidy(m <- nnet::multinom(direction ~ closeness, data=d)) |> 
  as_tibble() |> 
  mutate(estimate = plogis(estimate))
