...
TGv <- list.files(folder, "^[A-Z]{3}-(D|L)[0-9]\\.TextGrid$") # v for vector

for(t in TGv){
  tg <- tg.read(paste0(folder, t), encoding=detectEncoding(paste0(folder, t)))
...
files <- list.files(folder, "-all\\.TextGrid$")
...
files <- list.files(folder, "-f0.TextGrid")
