...
It seems that people that *liked* their partners more changed nose temperature *less*. But the effect was small and the residuals aren't normally distributed, so this isn't a reliable effect.
```{r}
...
summary(m <- lmer(tempChangeEntireExp ~ likeability + (1|dyad), datChange |> filter(ROI=="Nose")))
par(mfrow=c(2, 2))
hist(resid(m))
...
It seems that people in the *impersonal* condition changed eye temperature *more*. Again, effect small and residuals not normally distributed.
```{r}
summary(m <- lmer(tempChangeEntireExp ~ condition + (1|dyad), datChange |> filter(ROI=="Eyes")))
par(mfrow=c(2, 2))
hist(resid(m))
qqnorm(resid(m));qqline(resid(m))
plot(fitted(m), resid(m))
...
People that felt *closer* to their partner seemed to change eye temperature *less*. Again, small effect etc.
```{r}
summary(m <- lmer(tempChangeEntireExp ~ closeness + (1|dyad), datChange |> filter(ROI=="Eyes")))
par(mfrow=c(2, 2))
hist(resid(m))
qqnorm(resid(m));qqline(resid(m))
plot(fitted(m), resid(m))
...
People higher on *openness* seemed to change eye temperature *less*.
```{r}
summary(lmer(tempChangeEntireExp ~ extraversion + (1|dyad), datChange |> filter(ROI=="Eyes")))
summary(m <- lmer(tempChangeEntireExp ~ openness + (1|dyad), datChange |> filter(ROI=="Eyes")))
par(mfrow=c(2, 2))
hist(resid(m))
qqnorm(resid(m));qqline(resid(m))
plot(fitted(m), resid(m))
...
No effect with condition.
```{r}
summary(m <- lmer(tempChangeEntireExp ~ condition + (1|dyad), datChange |> filter(ROI=="Forehead")))
```

It seems that people that scored higher on *perceived similarity*, *liking their partner* and *wanting to become friends with them* changed forehead temperature *less*.
```{r}
summary(lmer(tempChangeEntireExp ~ closeness + (1|dyad), datChange |> filter(ROI=="Forehead")))
summary(m1 <- lmer(tempChangeEntireExp ~ similarity + (1|dyad), datChange |> filter(ROI=="Forehead")))
par(mfrow=c(2, 2))
hist(resid(m1))
qqnorm(resid(m1));qqline(resid(m1))
plot(fitted(m1), resid(m1))

summary(m2 <- lmer(tempChangeEntireExp ~ likeability + (1|dyad), datChange |> filter(ROI=="Forehead")))
par(mfrow=c(2, 2))
hist(resid(m2))
qqnorm(resid(m2));qqline(resid(m2))
plot(fitted(m2), resid(m2))

summary(m3 <- lmer(tempChangeEntireExp ~ becomeFriends + (1|dyad), datChange |> filter(ROI=="Forehead")))
par(mfrow=c(2, 2))
hist(resid(m3))
...
People higher on *openness* seemed to change forehead temperature *less*.
```{r}
...
It seems that people that scored higher on *liking their partner* changed cheek temperature *less*.
```{r}
